# Archivo: filter_dataset.py
# Este es archivo se encarga de filtrar el data set obtenido, generando uno nuevo balanceado.
# Autores: 
...
def filtrar(filename,newfilename): 
    try:
        ...
        r = 0 
        e = 0 
        f2.write("artist\tdanceability\tduration\tenery\tkey\tliveness\tloudness\tmode\tspeechiness\ttempo\ttime_signature\tgenre\n") 
        f2.write("d\tc\tc\tc\td\tc\tc\td\tc\tc\td\td\n")
        f2.write("\t\t\t\t\t\t\t\t\t\t\tclass\n")
        for line in data:
            if i > 2 :
                words = line.split('\t')
